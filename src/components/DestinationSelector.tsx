    ...
    return (
       <>
        <div className="py-8 flex justify-center items-center">
            <div className='w-40 h-40 md:w-80 md:h-80 xl:w-[480px] xl:h-[480px] relative'>
            {destinationImage.map(dest => (
                <img key={dest.title} className={
                clsx("absolute top-0 left-0 w-full h-full transition-all duration-300", selectedDestinations === dest.title ? 'opacity-100 rotate-0' : 'opacity-0 rotate-6')} src={dest.png} srcSet={dest.webp} alt={dest.title} />
            ))}
            </div>

        </div>

        <div className='flex flex-col gap-6 xl:max-w-[445px] xl:gap-10'>
            ...
            <div className='flex flex-col gap-6 md:flex-row'>
                <div className='w-full flex flex-col gap-3 overflow-hidden'>
                    <h2 className='font-sans-codensed tracking-wider text-blue-300 text-sm text-center xl:text-left'>AVG. DISTANCE</h2>
                    ...
                    <div className={clsx('w-full flex flex-row transition-transform duration-300',
                        ...
                        {avgDistance.map(dist => (
                            <p key={dist.title} className='block w-full flex-shrink-0 text-center font-serif text-3xl text-white xl:text-left'>{dist.avgDistance}</p>
                        ))}
                    </div>
                </div>

                <div className='w-full flex flex-col gap-3 overflow-hidden'>
                    <h2 className='font-sans-codensed tracking-wider text-blue-300 text-sm text-center xl:text-left'>EST. TRAVEL TIME</h2>
                    ...
                    <div className={clsx('w-full flex flex-row transition-transform duration-300',
                        ...
                        {travelTime.map(tTime => (
                            <p key={tTime.title} className='block w-full flex-shrink-0 text-center font-serif text-3xl text-white xl:text-left'>{tTime.travelTime}</p>
                        ))}
                    </div>
                </div>
            </div>
